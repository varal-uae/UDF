/*
 * TNRML-004-A05 — Bottom Bar to Rail Migrator
 * 
 * Global Reference ID: TNRML-004
 * Atomic Steps Reference ID: TNRML-004-A05
 * Atomic Step: Migrate the bottom bar's navigation entries into the rail's icon-based layout.
 * Tab Name: TNRML-004-A05 - UIUX | Row Tab Name: UDF
 * S.No: 7.0 | Sequence Order: 42817 | Assigned Team Member: Pooja | Group: UDF | Decision Group: Dashboard.
 ...
 * Target System: AISS Implementation — Bottom Bar to Rail Migrator
 * Backend Models: UserFlowNode (tbl_user_flow_nodes), UserFlowComplianceSummary (tbl_user_flow_compliance_summary), BQGraphCheckpointMetadata (tbl_bq_graph_checkpoint_metadata), DeadLetterQuarantine (q_dead_letter_quarantine), ExecutionAuditLog (tbl_execution_audit_log)
 * API Endpoint: POST /api/v1/navigation/bottom-bar-to-rail-migrator/validate/
...
/// Row 341: TNRML-004 (Seq 42817)
/// Action: Migrate the bottom bar's navigation entries into the rail's icon-based layout.
...
class BottomBarToRailMigratorPanel extends StatefulWidget {
  final String globalRefId;
  final String atomicStepRefId;
  final int sequenceOrder;

  const BottomBarToRailMigratorPanel({
    super.key,
    this.globalRefId = 'TNRML-004',
    this.atomicStepRefId = 'TNRML-004-A05',
    this.sequenceOrder = 42817,
  });

  @override
  State<BottomBarToRailMigratorPanel> createState() =>
      _BottomBarToRailMigratorPanelState();
}

class _BottomBarToRailMigratorPanelState
    extends State<BottomBarToRailMigratorPanel> {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        '${widget.globalRefId}: Bottom Bar to Rail Migrator',
              ...
              child: FilledButton.icon(
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                          'Bottom navigation bar entries mapped to NavigationRail icon destinations.'),
      ...
      home: Scaffold(
        body: SafeArea(
          child: SingleChildScrollView(
            child: BottomBarToRailMigratorPanel(),
          ),
        ),
      ),
    ),
