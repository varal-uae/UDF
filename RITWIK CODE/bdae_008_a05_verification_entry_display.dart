...
class Bdae008A05VerificationEntryDisplay {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-BDAE008A05-001: repo path must not be empty');
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-BDAE008A05-002: all 5 lock fields must be non-null',
    );
  ...
  static VerificationEntryEntry registerRule(VerificationEntryEntry entry) {
    assert(entry.lockStatus == LockStatus.active,
      'EC-BDAE008A05-003: lockStatus must be ACTIVE at display registration');
  ...
  static String bindToTarget(String ruleId, String lockedBy) {
    assert(ruleId.isNotEmpty, 'EC-BDAE008A05-005: FK bind requires valid ruleId');
...
class Bdae008A05VerificationEntryWidget extends StatelessWidget {
  ...
  Color _statusColor(LockStatus s) => switch (s) {
    LockStatus.active   => const Color(0xFF137333),
    LockStatus.released => const Color(0xFFE37400),
    LockStatus.expired  => const Color(0xFFD93025),
  };
