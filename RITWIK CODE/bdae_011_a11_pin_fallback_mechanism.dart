...
class PinFallbackEntry {
  ...
  }) : assert(pinExpiryMinutes <= 5,
         'EC-BDAE011A11-003: pinExpiryMinutes must be <= 5 per BDAE-011 rule set');
...
class Bdae011A11PinFallbackMechanism {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-BDAE011A11-001: repo path must not be empty');
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-BDAE011A11-002: all 5 PIN fallback fields must be non-null',
    );
  ...
  static PinFallbackEntry registerRule(PinFallbackEntry entry) {
    assert(entry.singleUseEnforced,
      'EC-BDAE011A11-003: singleUseEnforced must be TRUE — PIN must expire on first use');
    assert(entry.pinExpiryMinutes <= PinFallbackEntry.kMaxExpiryMin,
      'EC-BDAE011A11-003: pinExpiryMinutes must be <= 5');
  ...
  static String bindToTarget(String ruleId, String userId) {
    assert(ruleId.isNotEmpty, 'EC-BDAE011A11-005: FK bind requires valid ruleId');
...
class Bdae011A11PinFallbackWidget extends StatelessWidget {
  ...
  Color _pinColor(PinStatus s) => switch (s) {
    ...
    PinStatus.used      => const Color(0xFF137333),
    PinStatus.expired   => const Color(0xFFD93025),
  };
