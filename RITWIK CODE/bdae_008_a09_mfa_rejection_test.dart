...
class MfaTestEntry {
  ...
  }) : assert(testCoverage >= 0.0 && testCoverage <= 1.0,
         'EC-BDAE008A09-002: testCoverage must be 0.0–1.0');
...
class Bdae008A09MfaRejectionTest {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-BDAE008A09-001: repo path must not be empty');
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-BDAE008A09-002: all 5 MFA test fields must be non-null',
    );
  ...
  static MfaTestEntry registerRule(MfaTestEntry entry) {
    assert(entry.testCoverage >= 0.90,
      'EC-BDAE008A09-003: testCoverage must be >= 0.90 for gate');
  ...
  static String bindToTarget(String ruleId, String testType) {
    assert(ruleId.isNotEmpty, 'EC-BDAE008A09-005: FK bind requires valid ruleId');
...
class Bdae008A09MfaRejectionWidget extends StatelessWidget {
  ...
  Color _resultColor(TestResult r) => switch (r) {
    TestResult.pass   => const Color(0xFF137333),
    TestResult.fail   => const Color(0xFFD93025),
