...
// Description: Open backend services directory master_library/services/
...
// Date:        24-Sep-2026
// Source file: GEN-00796_open_services_dir.md
// ============================================================
...
// EC error codes: EC-GEN00796-001 through EC-GEN00796-UTL
...
/// The original Markdown source for GEN-00796.
/// Stored as a Dart constant so the pipeline scanner can index it.
/// Execute via [GEN-00796Executor.run()].
const String kGen00796SourceScript = r'''
# GEN-00796 — Open backend services directory master_library/services/
Metric: Directory Access · Complete/Not Complete

**Operational / verification step — not a source file.** Recorded for traceability.

## Action
Navigational/scoping step — target path `master_library/services/`. No source artifact; subsequent backend service steps write here.
''';
...
class Gen00796Executor {
  ...
  Map<String, dynamic> run() {
    if (traceId.isEmpty) {
      throw ArgumentError('EC-GEN00796-001: traceId required for GEN-00796');
    }
    if (originSourceId.isEmpty) {
      throw ArgumentError('EC-GEN00796-002: originSourceId required for GEN-00796');
    }
    return {
      'step_id':                  'GEN-00796',
      'source_language':          'Markdown',
      'source_script':            kGen00796SourceScript,
...
void main() {
  final executor = Gen00796Executor(
  ...
  print('GEN-00796 manifest ready:');
