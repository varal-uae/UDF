...
enum Slplu003A05ExecutionStatus { pending, running, complete, failed }
...
class Slplu003A05Pipeline {
  ...
  static Slplu003A05ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Slplu003A05ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-SLPLU003A05-VAL',
      );
    }
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-SLPLU003A05-000: configs must not be empty for SLPLU-003-A05');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-SLPLU003A05-TRI: triangular check failed for SLPLU-003-A05');
    }
...
void main() async {
  final configs = [
    Slplu003A05Config(
      ...
      tokenName: 'slplu-003-a05_tokenName',
      tokenValue: 'slplu-003-a05_tokenValue',
      tokenCategory: 'slplu-003-a05_tokenCategory',
      appliedComponent: 'slplu-003-a05_appliedComponent',
  ...
  final out = await Slplu003A05Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('SLPLU-003-A05 [Complete / Partial / Not Complete] → $out');
}
