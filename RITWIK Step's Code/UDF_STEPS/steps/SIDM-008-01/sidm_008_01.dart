// ============================================================
// SIDM-008-01 — SIDM System Module
...
enum Sidm00801ExecutionStatus { pending, running, complete, failed }
...
class Sidm00801ValidationResult {
  ...
  String get conformanceOutput {
    switch (conformanceLevel) {
      case Sidm00801ConformanceLevel.good:    return 'Good';
      case Sidm00801ConformanceLevel.average: return 'Average';
      case Sidm00801ConformanceLevel.poor:    return 'Poor';
    }
  }
}
...
class Sidm00801Pipeline {
  ...
  static Sidm00801Config _ec1Locates(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-001: gateId required for SIDM-008-01');
    }
    // the SIDM-008-01 configuration in the source repository
    return config;
  }

  // EC:2 — System extracts gateId and checkRule from the SIDM-008-01 registry.
  static Sidm00801Config _ec2Extracts(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-002: gateId required for SIDM-008-01');
    }
    // gateId and checkRule from the SIDM-008-01 registry
    return config;
  }
  ...
  static Sidm00801Config _ec3Compiles(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-003: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801Config _ec4Validates(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-004: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801Config _ec5Registers(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-005: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801Config _ec6Validates(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-006: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801Config _ec7Routes(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-007: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801Config _ec8Publishes(Sidm00801Config config) {
    if (config.gateId.isEmpty) {
      throw ArgumentError(
          'EC-SIDM00801-008: gateId required for SIDM-008-01');
    }
  ...
  static Sidm00801ValidationResult calculateConformance({
    ...
    final level = rate >= _optimal
        ? Sidm00801ConformanceLevel.good
        : rate >= _floor
            ? Sidm00801ConformanceLevel.average
            : Sidm00801ConformanceLevel.poor;
    return Sidm00801ValidationResult(
...
void main() async {
  final configs = [
    Sidm00801Config(
      ...
      gateId: 'sidm-008-01_gateId',
      checkRule: 'sidm-008-01_checkRule',
      passThreshold: 'sidm-008-01_passThreshold',
      failureReason: 'sidm-008-01_failureReason',
  ...
  final out = await Sidm00801Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('SIDM-008-01 [Good / Average / Poor] → $out');
}
