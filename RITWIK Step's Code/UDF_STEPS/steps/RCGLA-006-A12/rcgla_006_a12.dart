...
// Metric:       Modal/Overlay Interaction Compliance
...
// Step No:      912 of 1073
// ============================================================
// Why:          Eliminates unreadable columns and text truncation on small mobile screens.
// Mobile:       Establishes card stacking as the baseline layout, forcing larger displays to build on stacked rows.
...
enum Rcgla006A12ExecutionStatus { pending, running, complete, failed }
...
class Rcgla006A12ValidationResult {
  ...
  String get conformanceOutput {
    switch (conformanceLevel) {
      case Rcgla006A12ConformanceLevel.good:    return 'Good';
      case Rcgla006A12ConformanceLevel.average: return 'Average';
      case Rcgla006A12ConformanceLevel.poor:    return 'Poor';
    }
  }
}
...
class Rcgla006A12Pipeline {
  ...
  static Rcgla006A12ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Rcgla006A12ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-RCGLA006A12-VAL',
      );
    }
    ...
    final level = rate >= _optimal
        ? Rcgla006A12ConformanceLevel.good
        : rate >= _floor
            ? Rcgla006A12ConformanceLevel.average
            : Rcgla006A12ConformanceLevel.poor;
    return Rcgla006A12ValidationResult(
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-RCGLA006A12-000: configs must not be empty for RCGLA-006-A12');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-RCGLA006A12-TRI: triangular check failed for RCGLA-006-A12');
    }
...
void main() async {
  final configs = [
    Rcgla006A12Config(
      ...
      gridColumns: 'rcgla-006-a12_gridColumns',
      gutterSizePx: 'rcgla-006-a12_gutterSizePx',
      maxWidthPx: 'rcgla-006-a12_maxWidthPx',
      breakpointLabel: 'rcgla-006-a12_breakpointLabel',
  ...
  final out = await Rcgla006A12Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('RCGLA-006-A12 [Good / Average / Poor] → $out');
}
