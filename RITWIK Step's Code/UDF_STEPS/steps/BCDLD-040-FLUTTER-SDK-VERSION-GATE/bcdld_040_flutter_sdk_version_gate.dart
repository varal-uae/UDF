...
class Bcdld040FlutterSdkVersionGate {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
      ...
      throw ArgumentError('EC-BCDLD040-001: repo path must not be empty');
    };
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    if (!(required.every((k) => config.containsKey(k) && config[k] != null))) {
      throw ArgumentError('EC-BCDLD040-002: all 5 SDK lock fields must be non-null',
    );
  ...
  static FlutterSdkLockEntry registerRule(FlutterSdkLockEntry entry) {
        if (!(entry.isExactVersion)) {
      throw ArgumentError('EC-BCDLD040-003: flutterVersion "${entry.flutterVersion}" is not exact semver');
    }
    };
        if (entry.dartSdkConstraint.isEmpty) {
      throw ArgumentError('EC-BCDLD040-003: dartSdkConstraint must be declared');
    };
        if (!(entry.channel == SdkChannel.stable)) {
      throw ArgumentError('EC-BCDLD040-003: production channel must be stable, got ${entry.channelLabel}');
    };
  ...
  static String bindToTarget(String ruleId, String pubspecPath) {
      ...
      throw ArgumentError('EC-BCDLD040-005: FK bind requires valid ruleId');
    };
