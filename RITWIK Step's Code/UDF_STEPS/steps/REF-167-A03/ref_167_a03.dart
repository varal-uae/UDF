...
class Ref167A03ValidationResult {
  final int    totalRecords;
  final int    conformantRecords;
  final int    violationCount;
  final double conformanceRate;
  final Ref167A03ConformanceLevel conformanceLevel;
  final bool   gatePass;
  final String ecLineRef;

  const Ref167A03ValidationResult({
...
class Ref167A03Pipeline {
  ...
  static Ref167A03Config _ec1Execute(Ref167A03Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-REF167A03-001: colorToken required for REF-167-A03');
    }
  ...
  static Ref167A03Config _ec2Execute(Ref167A03Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-REF167A03-002: colorToken required for REF-167-A03');
    }
  ...
  static Ref167A03Config _ec3Execute(Ref167A03Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-REF167A03-003: colorToken required for REF-167-A03');
    }
  ...
  static Ref167A03Config _ec4Execute(Ref167A03Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-REF167A03-004: colorToken required for REF-167-A03');
    }
  ...
  static Ref167A03ValidationResult calculateConformance({
    ...
    return Ref167A03ValidationResult(
      ...
      ecLineRef:         'EC-REF167A03-VAL',
    );
  }

  static Ref167A03Config routeToRegistry(
    Ref167A03Config config,
    Ref167A03ValidationResult result,
  ) {
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-REF167A03-000: configs must not be empty for REF-167-A03');
    }
...
class Ref167A03Widget extends StatelessWidget {
  final List<Ref167A03Config> configs;
  const Ref167A03Widget({super.key, required this.configs});
...
void main() async {
  final configs = [
    Ref167A03Config(
      configId: 'ref167a03-cfg-001',
      colorToken: 'ref-167-a03_colorToken',
      hexValue: 'ref-167-a03_hexValue',
      wcagRatio: 'ref-167-a03_wcagRatio',
      usageContext: 'ref-167-a03_usageContext',
      traceId:                 'trace-ref167a03-001',
      originSourceId:          'origin-ref167a03',
      immediatePredecessorId:  'pred-ref167a03-001',
