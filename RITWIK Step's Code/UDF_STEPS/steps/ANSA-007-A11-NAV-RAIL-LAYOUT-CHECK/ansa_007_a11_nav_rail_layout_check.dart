...
class Ansa007A11NavRailLayoutCheck {
  ...
  // EC:1 — Locate the Navigation Rail layout configuration within the nav-rail-kit source repository.  // error: EC-ANSA007A11-001
  static Map<String, dynamic>? locateConfiguration(String componentRef) {
        if (!(componentRef == 'ANSA-007-A11')) {
      throw ArgumentError('Invalid component ref');
    };
    return {};
  }

  // EC:2 — Extract railWidth, destinationCount, labelVisibility, breakpointMode, groupAlignment from rail_layout_config_registry.  // error: EC-ANSA007A11-002
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    return Map<String, dynamic>.from(config);
  }

  // EC:3 — Compile MD3 Navigation Rail rule set: width=80dp (rail), destinations 3-7, labels always-visible on expanded.  // error: EC-ANSA007A11-003
  ...
  // EC:4 — Register compiled MD3 rail rule set as immutable entry in rail_layout_config_registry.  // error: EC-ANSA007A11-004
  ...
  // EC:5 — Bind each registered rail rule to NavigationRail scaffold slot by applying rail_scaffold_slot FK constraint.  // error: EC-ANSA007A11-005
  static String bindToTarget(String ruleId, String targetSlot) {
    return '$targetSlot:$ruleId';
  }

  // EC:6 — Validate bound rail configuration by executing layout conformance check confirming width, destination count, label spec.  // error: EC-ANSA007A11-006
  ...
  // EC:7 — Validate configured rail implementation against Design Fidelity metric threshold (Good >= 95% conformance).  // error: EC-ANSA007A11-007
  static String evaluateMetric(double actual) {
    return actual <= _threshold ? 'PASS' : 'FAIL';
  }

  // EC:8 — Route validated rail layout configuration to shared_nav_utils npm package as authoritative Rail Layout Registry entry.  // error: EC-ANSA007A11-008
