...
// Metric:       Floating Action Button (Thumb-Zone) Ergonomics
...
enum Sgtim019A02ExecutionStatus { pending, running, complete, failed }
...
class Sgtim019A02ValidationResult {
  ...
  String get conformanceOutput {
    switch (conformanceLevel) {
      case Sgtim019A02ConformanceLevel.good:    return 'Good';
      case Sgtim019A02ConformanceLevel.average: return 'Average';
      case Sgtim019A02ConformanceLevel.poor:    return 'Poor';
    }
  }
}
...
class Sgtim019A02Pipeline {
  ...
  static Sgtim019A02ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Sgtim019A02ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-SGTIM019A02-VAL',
      );
    }
    ...
    final level = rate >= _optimal
        ? Sgtim019A02ConformanceLevel.good
        : rate >= _floor
            ? Sgtim019A02ConformanceLevel.average
            : Sgtim019A02ConformanceLevel.poor;
    return Sgtim019A02ValidationResult(
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-SGTIM019A02-000: configs must not be empty for SGTIM-019-A02');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-SGTIM019A02-TRI: triangular check failed for SGTIM-019-A02');
    }
...
void main() async {
  final configs = [
    Sgtim019A02Config(
      ...
      componentId: 'sgtim-019-a02_componentId',
      targetSizeDp: 'sgtim-019-a02_targetSizeDp',
      actualSizeDp: 'sgtim-019-a02_actualSizeDp',
      complianceStatus: 'sgtim-019-a02_complianceStatus',
  ...
  final out = await Sgtim019A02Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('SGTIM-019-A02 [Good / Average / Poor] → $out');
}
