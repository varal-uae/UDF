...
enum Nskfi003A05ExecutionStatus {
  pending,
  running,
  complete,
  failed,
...
/// Configuration record for NSKFI-003-A05.
...
class Nskfi003A05Pipeline {
  ...
  static Nskfi003A05Config _ec1Execute(Nskfi003A05Config config) {
      ...
      throw ArgumentError('EC-NSKFI003A05-001: configId required for NSKFI-003-A05');
    };
  ...
  static Nskfi003A05Config _ec2Execute(Nskfi003A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-NSKFI003A05-002: configId required for NSKFI-003-A05');
    };
  ...
  static Nskfi003A05Config _ec3Execute(Nskfi003A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-NSKFI003A05-003: configId required for NSKFI-003-A05');
    };
  ...
  static Nskfi003A05Config _ec4Execute(Nskfi003A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-NSKFI003A05-004: configId required for NSKFI-003-A05');
    };
  ...
  static Nskfi003A05ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return const Nskfi003A05ValidationResult(
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      return {'error': 'EC-NSKFI003A05-001: empty config list', 'dlq': true};
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      return {'error': 'EC-NSKFI003A05-TRI: triangular check failed', 'dlq': true};
    }
...
void main() async {
  final configs = [
    Nskfi003A05Config(
      ...
      ruleKey:                 'nskfi-003-a05_rule',
      ruleValue:               'nskfi-003-a05_value',
  ...
  final result = await Nskfi003A05Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('NSKFI-003-A05 → $result');
}
