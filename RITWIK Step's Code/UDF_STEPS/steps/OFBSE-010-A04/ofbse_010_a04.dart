...
enum Ofbse010A04ExecutionStatus { pending, running, complete, failed }
...
class Ofbse010A04Pipeline {
  ...
  static Ofbse010A04Config _ec1Execute(Ofbse010A04Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-OFBSE010A04-001: colorToken required for OFBSE-010-A04');
    }
    // Drop timer limits
    return config;
  }
  ...
  static Ofbse010A04Config _ec2Execute(Ofbse010A04Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-OFBSE010A04-002: colorToken required for OFBSE-010-A04');
    }
  ...
  static Ofbse010A04Config _ec3Execute(Ofbse010A04Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-OFBSE010A04-003: colorToken required for OFBSE-010-A04');
    }
  ...
  static Ofbse010A04Config _ec4Execute(Ofbse010A04Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-OFBSE010A04-004: colorToken required for OFBSE-010-A04');
    }
    // Warning color maps
    return config;
  }
  ...
  static Ofbse010A04ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Ofbse010A04ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-OFBSE010A04-VAL',
      );
    }
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-OFBSE010A04-000: configs must not be empty for OFBSE-010-A04');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-OFBSE010A04-TRI: triangular check failed for OFBSE-010-A04');
    }
...
void main() async {
  final configs = [
    Ofbse010A04Config(
      ...
      colorToken: 'ofbse-010-a04_colorToken',
      hexValue: 'ofbse-010-a04_hexValue',
      wcagRatio: 'ofbse-010-a04_wcagRatio',
      usageContext: 'ofbse-010-a04_usageContext',
  ...
  final out = await Ofbse010A04Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('OFBSE-010-A04 [Complete / Partial / Not Complete] → $out');
}
