...
class Pelce01908ValidationResult {
  final int    totalRecords;
  final int    conformantRecords;
  final int    violationCount;
  final double conformanceRate;
  final Pelce01908ConformanceLevel conformanceLevel;
  final bool   gatePass;
  final String ecLineRef;

  const Pelce01908ValidationResult({
  ...
  String get conformanceOutput {
    switch (conformanceLevel) {
      case Pelce01908ConformanceLevel.complete:    return 'Complete';
      case Pelce01908ConformanceLevel.partial:     return 'Partial';
      case Pelce01908ConformanceLevel.notComplete: return 'Not Complete';
    }
  }
}
...
class Pelce01908Pipeline {
  ...
  static Pelce01908Config _ec1Locates(Pelce01908Config config) {
    if (config.ruleKey.isEmpty) {
      throw ArgumentError(
          'EC-PELCE01908-001: ruleKey required for PELCE-019-08');
    }
    // the PELCE-019-08 configuration in the source repository
    return config;
  }

  // EC:2 — System extracts ruleKey and ruleValue from the PELCE-019-08 registry.
  static Pelce01908Config _ec2Extracts(Pelce01908Config config) {
    if (config.ruleKey.isEmpty) {
      throw ArgumentError(
          'EC-PELCE01908-002: ruleKey required for PELCE-019-08');
    }
    // ruleKey and ruleValue from the PELCE-019-08 registry
    return config;
  }
  ...
  static Pelce01908Config _ec5Registers(Pelce01908Config config) {
    if (config.ruleKey.isEmpty) {
      throw ArgumentError(
          'EC-PELCE01908-005: ruleKey required for PELCE-019-08');
    }
  ...
  static Pelce01908Config _ec7Routes(Pelce01908Config config) {
    if (config.ruleKey.isEmpty) {
      throw ArgumentError(
          'EC-PELCE01908-007: ruleKey required for PELCE-019-08');
    }
  ...
  static Pelce01908Config _ec8Publishes(Pelce01908Config config) {
    if (config.ruleKey.isEmpty) {
      throw ArgumentError(
          'EC-PELCE01908-008: ruleKey required for PELCE-019-08');
    }
  ...
  static Pelce01908ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return const Pelce01908ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-PELCE01908-VAL',
      );
    }
    ...
    return Pelce01908ValidationResult(
      ...
      ecLineRef:         'EC-PELCE01908-VAL',
    );
  }

  static Pelce01908Config routeToRegistry(
    Pelce01908Config config,
    Pelce01908ValidationResult result,
  ) {
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-PELCE01908-000: configs must not be empty for PELCE-019-08');
    }
...
class Pelce01908Widget extends StatelessWidget {
  final List<Pelce01908Config> configs;
  const Pelce01908Widget({super.key, required this.configs});
...
void main() async {
  final configs = [
    Pelce01908Config(
      configId: 'pelce01908-cfg-001',
      ruleKey: 'pelce-019-08_ruleKey',
      ruleValue: 'pelce-019-08_ruleValue',
      metricLabel: 'pelce-019-08_metricLabel',
      complianceTarget: 'pelce-019-08_complianceTarget',
      traceId:                 'trace-pelce01908-001',
      originSourceId:          'origin-pelce01908',
      immediatePredecessorId:  'pred-pelce01908-001',
