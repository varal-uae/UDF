...
enum Ttmac026A06ExecutionStatus { pending, running, complete, failed }
...
class Ttmac026A06Pipeline {
  ...
  static Ttmac026A06ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Ttmac026A06ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-TTMAC026A06-VAL',
      );
    }
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-TTMAC026A06-000: configs must not be empty for TTMAC-026-A06');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-TTMAC026A06-TRI: triangular check failed for TTMAC-026-A06');
    }
...
void main() async {
  final configs = [
    Ttmac026A06Config(
      ...
      componentId: 'ttmac-026-a06_componentId',
      targetSizeDp: 'ttmac-026-a06_targetSizeDp',
      actualSizeDp: 'ttmac-026-a06_actualSizeDp',
      complianceStatus: 'ttmac-026-a06_complianceStatus',
  ...
  final out = await Ttmac026A06Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('TTMAC-026-A06 [Complete / Partial / Not Complete] → $out');
}
