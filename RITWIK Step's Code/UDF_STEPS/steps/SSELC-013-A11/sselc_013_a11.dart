...
// Data requirement: Program the core framework to programmatically ignore local styling overrides.
// ============================================================
...
enum Sselc013A11ExecutionStatus {
  pending,
  running,
  complete,
  failed,
...
/// Configuration record for SSELC-013-A11.
...
class Sselc013A11Pipeline {
  ...
  static Sselc013A11Config _ec1Execute(Sselc013A11Config config) {
    if (config.layoutId.isEmpty) {
      throw ArgumentError(
          'EC-SSELC013A11-001: layoutId required for SSELC-013-A11');
    }
  ...
  static Sselc013A11Config _ec2Execute(Sselc013A11Config config) {
    if (config.layoutId.isEmpty) {
      throw ArgumentError(
          'EC-SSELC013A11-002: layoutId required for SSELC-013-A11');
    }
  ...
  static Sselc013A11Config _ec3Execute(Sselc013A11Config config) {
    if (config.layoutId.isEmpty) {
      throw ArgumentError(
          'EC-SSELC013A11-003: layoutId required for SSELC-013-A11');
    }
  ...
  static Sselc013A11Config _ec4Execute(Sselc013A11Config config) {
    if (config.layoutId.isEmpty) {
      throw ArgumentError(
          'EC-SSELC013A11-004: layoutId required for SSELC-013-A11');
    }
  ...
  static Sselc013A11ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return const Sselc013A11ValidationResult(
        totalRecords: 0, conformantRecords: 0, violationCount: 0,
        conformanceRate: 0.0,
        conformanceLevel: Sselc013A11ConformanceLevel.notComplete,
    ...
    final level      = rate >= _optimal
        ? Sselc013A11ConformanceLevel.complete
        : rate >= _floor
            ? Sselc013A11ConformanceLevel.partial
            : Sselc013A11ConformanceLevel.notComplete;
...
void main() async {
  final configs = [
    Sselc013A11Config(
      ...
      layoutId: 'sselc-013-a11_layoutId',
      splitRatio: 'sselc-013-a11_splitRatio',
      containerWidth: 'sselc-013-a11_containerWidth',
      breakpointKey: 'sselc-013-a11_breakpointKey',
  ...
  final result = await Sselc013A11Pipeline.run(
    configs: configs, userId: 'ritwik-udf');
  print('SSELC-013-A11 → $result');
}
