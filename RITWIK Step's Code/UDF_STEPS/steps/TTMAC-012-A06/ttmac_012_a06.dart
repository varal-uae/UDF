...
// Metric:       Corner Radius Token Consistency
...
enum Ttmac012A06ExecutionStatus { pending, running, complete, failed }
...
class Ttmac012A06ValidationResult {
  ...
  String get conformanceOutput {
    switch (conformanceLevel) {
      case Ttmac012A06ConformanceLevel.pass_: return 'Pass';
      case Ttmac012A06ConformanceLevel.fail_: return 'Fail';
    }
  }
}
...
class Ttmac012A06Pipeline {
  ...
  static Ttmac012A06Config _ec1Execute(Ttmac012A06Config config) {
    if (config.gridColumns.isEmpty) {
      throw ArgumentError(
          'EC-TTMAC012A06-001: gridColumns required for TTMAC-012-A06');
    }
  ...
  static Ttmac012A06Config _ec2Execute(Ttmac012A06Config config) {
    if (config.gridColumns.isEmpty) {
      throw ArgumentError(
          'EC-TTMAC012A06-002: gridColumns required for TTMAC-012-A06');
    }
  ...
  static Ttmac012A06Config _ec3Execute(Ttmac012A06Config config) {
    if (config.gridColumns.isEmpty) {
      throw ArgumentError(
          'EC-TTMAC012A06-003: gridColumns required for TTMAC-012-A06');
    }
  ...
  static Ttmac012A06Config _ec4Execute(Ttmac012A06Config config) {
    if (config.gridColumns.isEmpty) {
      throw ArgumentError(
          'EC-TTMAC012A06-004: gridColumns required for TTMAC-012-A06');
    }
  ...
  static Ttmac012A06ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Ttmac012A06ValidationResult(
        ...
        conformanceRate: 0.0,
        conformanceLevel: Ttmac012A06ConformanceLevel.fail_,
        gatePass: false, ecLineRef: 'EC-TTMAC012A06-VAL',
      );
    }
    ...
    final level = rate >= _floor
        ? Ttmac012A06ConformanceLevel.pass_
        : Ttmac012A06ConformanceLevel.fail_;
    return Ttmac012A06ValidationResult(
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-TTMAC012A06-000: configs must not be empty for TTMAC-012-A06');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-TTMAC012A06-TRI: triangular check failed for TTMAC-012-A06');
    }
...
void main() async {
  final configs = [
    Ttmac012A06Config(
      ...
      gridColumns: 'ttmac-012-a06_gridColumns',
      gutterSizePx: 'ttmac-012-a06_gutterSizePx',
      maxWidthPx: 'ttmac-012-a06_maxWidthPx',
      breakpointLabel: 'ttmac-012-a06_breakpointLabel',
  ...
  final out = await Ttmac012A06Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('TTMAC-012-A06 [Pass / Fail] → $out');
}
