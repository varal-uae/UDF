...
enum Rcgla023A05ExecutionStatus {
  pending,
  running,
  complete,
  failed,
...
/// Configuration record for RCGLA-023-A05.
...
class Rcgla023A05Pipeline {
  ...
  static Rcgla023A05Config _ec1Locates(Rcgla023A05Config config) {
      ...
      throw ArgumentError('EC-RCGLA023A05-001: configId required for RCGLA-023-A05');
    };
    // the RCGLA-023-A05 configuration in the source repo
    return config;
  }

  // EC:2 — System extracts required data fields from the RCGLA-023-A05 registry.
  static Rcgla023A05Config _ec2Extracts(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-002: configId required for RCGLA-023-A05');
    };
    // required data fields from the RCGLA-023-A05 regist
    return config;
  }
  ...
  static Rcgla023A05Config _ec3Compiles(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-003: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05Config _ec4Registers(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-004: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05Config _ec5Validates(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-005: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05Config _ec6Routes(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-006: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05Config _ec7Writes(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-007: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05Config _ec8Publishes(Rcgla023A05Config config) {
        if (config.configId.isEmpty) {
      throw ArgumentError('EC-RCGLA023A05-008: configId required for RCGLA-023-A05');
    };
  ...
  static Rcgla023A05ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return const Rcgla023A05ValidationResult(
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      return {'error': 'EC-RCGLA023A05-001: empty config list', 'dlq': true};
    }
    ...
    if (!triangularCheck(configs.length, p8.length)) {
      return {'error': 'EC-RCGLA023A05-TRI: triangular check failed', 'dlq': true};
    }
...
void main() async {
  final configs = [
    Rcgla023A05Config(
      ...
      ruleKey:                 'rcgla-023-a05_rule',
      ruleValue:               'rcgla-023-a05_value',
  ...
  final result = await Rcgla023A05Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('RCGLA-023-A05 → $result');
}
