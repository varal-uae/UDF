...
// Data requirement: Add interactive hover tooltip box displaying precise latency breakdown (network, API gateway, DB que
// ============================================================
...
enum Is36Slplu012As01A11ExecutionStatus {
  pending,
  running,
  complete,
  failed,
...
/// Configuration record for IS36-SLPLU-012-AS01-A11.
...
/// IS36-SLPLU-012-AS01-A11: Lineage Trace Time Line Chart (System Latency)
///
...
class Is36Slplu012As01A11Pipeline {
  ...
  static Is36Slplu012As01A11Config _ec1Execute(Is36Slplu012As01A11Config config) {
    if (config.chartId.isEmpty) {
      throw ArgumentError(
          'EC-IS36SLPLU012-001: chartId required for IS36-SLPLU-012-AS01-A11');
    }
    // Query latency logs
    return config;
  }
  ...
  static Is36Slplu012As01A11Config _ec2Execute(Is36Slplu012As01A11Config config) {
    if (config.chartId.isEmpty) {
      throw ArgumentError(
          'EC-IS36SLPLU012-002: chartId required for IS36-SLPLU-012-AS01-A11');
    }
    // Render line chart
    return config;
  }
  ...
  static Is36Slplu012As01A11Config _ec3Execute(Is36Slplu012As01A11Config config) {
    if (config.chartId.isEmpty) {
      throw ArgumentError(
          'EC-IS36SLPLU012-003: chartId required for IS36-SLPLU-012-AS01-A11');
    }
  ...
  static Is36Slplu012As01A11Config _ec4Execute(Is36Slplu012As01A11Config config) {
    if (config.chartId.isEmpty) {
      throw ArgumentError(
          'EC-IS36SLPLU012-004: chartId required for IS36-SLPLU-012-AS01-A11');
    }
    // Code alert trigger
    return config;
  }
  ...
  static Is36Slplu012As01A11ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return const Is36Slplu012As01A11ValidationResult(
        totalRecords: 0, conformantRecords: 0, violationCount: 0,
        conformanceRate: 0.0,
        conformanceLevel: Is36Slplu012As01A11ConformanceLevel.notComplete,
        gatePass: false,
    ...
    final level      = rate >= _optimal
        ? Is36Slplu012As01A11ConformanceLevel.complete
        : rate >= _floor
            ? Is36Slplu012As01A11ConformanceLevel.partial
            : Is36Slplu012As01A11ConformanceLevel.notComplete;
...
void main() async {
  final configs = [
    Is36Slplu012As01A11Config(
      ...
      chartId: 'is36-slplu-012-as01-a11_chartId_value',
      dataSource: 'is36-slplu-012-as01-a11_dataSource_value',
      metricLabel: 'is36-slplu-012-as01-a11_metricLabel_value',
      refreshIntervalMs: 'is36-slplu-012-as01-a11_refreshIntervalMs_value',
  ...
  final result = await Is36Slplu012As01A11Pipeline.run(
    configs: configs, userId: 'ritwik-udf');
  print('IS36-SLPLU-012-AS01-A11 → $result');
}
