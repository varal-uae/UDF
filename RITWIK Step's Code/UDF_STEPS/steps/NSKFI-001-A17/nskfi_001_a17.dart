...
enum Nskfi001A17ExecutionStatus { pending, running, complete, failed }
...
class Nskfi001A17Pipeline {
  ...
  static Nskfi001A17Config _ec1Execute(Nskfi001A17Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-NSKFI001A17-001: colorToken required for NSKFI-001-A17');
    }
  ...
  static Nskfi001A17Config _ec2Execute(Nskfi001A17Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-NSKFI001A17-002: colorToken required for NSKFI-001-A17');
    }
  ...
  static Nskfi001A17Config _ec3Execute(Nskfi001A17Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-NSKFI001A17-003: colorToken required for NSKFI-001-A17');
    }
  ...
  static Nskfi001A17Config _ec4Execute(Nskfi001A17Config config) {
    if (config.colorToken.isEmpty) {
      throw ArgumentError(
          'EC-NSKFI001A17-004: colorToken required for NSKFI-001-A17');
    }
  ...
  static Nskfi001A17ValidationResult calculateConformance({
    ...
    if (configs.isEmpty) {
      return Nskfi001A17ValidationResult(
        ...
        gatePass: false, ecLineRef: 'EC-NSKFI001A17-VAL',
      );
    }
  ...
  static Future<Map<String, dynamic>> run({
    ...
    if (configs.isEmpty) {
      throw ArgumentError('EC-NSKFI001A17-000: configs must not be empty for NSKFI-001-A17');
    }
    ...
    if (!triangularCheck(configs.length, p4.length)) {
      throw ArgumentError('EC-NSKFI001A17-TRI: triangular check failed for NSKFI-001-A17');
    }
...
void main() async {
  final configs = [
    Nskfi001A17Config(
      ...
      colorToken: 'nskfi-001-a17_colorToken',
      hexValue: 'nskfi-001-a17_hexValue',
      wcagRatio: 'nskfi-001-a17_wcagRatio',
      usageContext: 'nskfi-001-a17_usageContext',
  ...
  final out = await Nskfi001A17Pipeline.run(configs: configs, userId: 'ritwik-udf');
  print('NSKFI-001-A17 [Complete / Partial / Not Complete] → $out');
}
