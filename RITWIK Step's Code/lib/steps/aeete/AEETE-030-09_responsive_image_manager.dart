...
class ImageProtocolRule {
  ...
  /// EC:3 — Compute pixel height from aspect ratio + width.
...
class ResponsiveImageManager {
  ...
  // -------------------------------------------------------------------------
  // EC:3 — Compile protocol rules for all 5 asset types.
  // -------------------------------------------------------------------------
  List<ImageProtocolRule> compileRules() =>
      kImageProtocolRules.values.toList();

  // -------------------------------------------------------------------------
  // EC:6 — CLS compliance check for one asset at one viewport.
...
class _ResponsiveImageState extends State<ResponsiveImage> {
  ...
  // EC:3 — Compute reserved dimensions to prevent CLS.
  ...
  Widget build(BuildContext context) {
    return Semantics(
      ...
      image: true,
      child: SizedBox(
        // EC:5 — Explicit dimensions reserved (prevents layout shift)
        width:  _width.toDouble(),
        height: _height.toDouble(),
        child: Stack(
          children: [
            // EC:5 — Skeleton rendered while image loads
