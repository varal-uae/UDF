...
class SecondaryValidationActionEntry {
  ...
  })  : assert(!totpSecretRef.startsWith('RAW:'),
            'EC-BDAE008A04-003: totpSecretRef must be a Secret Manager path — never a raw secret'),
        assert(pauseLatencyMs >= 0,
            'EC-BDAE008A04-002: pauseLatencyMs must be >= 0');
...
class Bdae008A04WorkflowPauseGate {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-BDAE008A04-001: repo path must not be empty');
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-BDAE008A04-002: all 5 workflow pause fields must be non-null',
    );
  ...
  static String bindToTarget(String ruleId, String actionTag) {
    assert(ruleId.isNotEmpty, 'EC-BDAE008A04-005: FK bind requires valid ruleId');
...
class Bdae008A04WorkflowPauseWidget extends StatelessWidget {
  ...
  Color _statusColor(ActionStatus s) => switch (s) {
    ...
    ActionStatus.verified => const Color(0xFF137333),
    ActionStatus.rejected => const Color(0xFFD93025),
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 11,
                          color: latencyOk ? null : const Color(0xFFD93025),
                        ),
                      ),
                      Text(
