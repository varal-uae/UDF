...
class MtoiExceptionEntry {
  ...
  const MtoiExceptionEntry({
    ...
    required this.transformationLogicHash,
           ...
           'EC-BLGTA008-06-003: zoomMin must be >= 2.0'),
        assert(contrastRatioMin >= 4.5,
           'EC-BLGTA008-06-003: contrastRatioMin must be >= 4.5 (WCAG AA)');
  ...
  /// EC:6 gate — constraints met AND resolution within floor
  ...
  Color get handlingColor => switch (handlingOutput) {
    MtouHandlingOutput.good    => const Color(0xFF137333),
    MtouHandlingOutput.average => const Color(0xFFE37400),
    MtouHandlingOutput.poor    => const Color(0xFFD93025),
  };
...
class Blgta00806MtoiExceptionHandler {
  ...
  // EC:1 — Locate MTOI exception handler config in blgta-008-kit repo.
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-BLGTA008-06-001: repo path must not be empty');
    return {'ref': 'BLGTA-008-06', 'config_file': 'blgta-008-06.yaml'};
  }

  // EC:2 — Extract mtoiRuleId, mobilePlatform, osVersion, deviceType, screenDimensions.
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-BLGTA008-06-002: all 5 MTOI device fields must be non-null',
    );
    return Map<String, dynamic>.from(config);
  }

  // EC:3 — Compile MTOI interface rule set:
  ...
  // EC:4 — Register compiled MTOI rule set as immutable in
  //         mtoi_exception_registry with immutable_IND=TRUE.
  static MtoiExceptionEntry registerRule(MtoiExceptionEntry entry) {
    assert(entry.zoomMin >= MtoiExceptionEntry.kMinZoom,
      'EC-BLGTA008-06-003: zoomMin < 2.0');
    assert(entry.contrastRatioMin >= MtoiExceptionEntry.kMinContrast,
      'EC-BLGTA008-06-003: contrastRatioMin < 4.5 (WCAG AA)');
  ...
  // EC:5 — Bind each MTOI rule to OCR failure handler
  //         via ocr_failure_handler_FK constraint.
  static String bindToTarget(String ruleId, String mobilePlatform) {
    assert(ruleId.isNotEmpty, 'EC-BLGTA008-06-005: FK bind requires valid ruleId');
    return '$mobilePlatform:$ruleId';
  }

  // EC:6 — Validate: single-column renders, zoom >= 2x, contrast >= 4.5,
  ...
  // EC:7 — Validate against MTOI Exception Handling Time metric.
  //         Floor=<=60min; Optimal=<=30min; Ceiling=<=15min (BPO SLA).
  static String evaluateMetric(MtoiExceptionScanResult scan) {
    return scan.violationCount == 0 ? 'PASS' : 'FAIL';
  }

  // EC:8 — Route validated MTOI config to blgta_rule_registry
