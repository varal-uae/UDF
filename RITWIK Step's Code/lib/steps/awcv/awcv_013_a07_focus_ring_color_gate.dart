...
class FocusRingColorEntry {
  ...
  }) : assert(focusRingWidthPx >= 2,
         'EC-AWCV013A07-003: focusRingWidthPx must be >= 2px');
...
class Awcv013A07FocusRingColorGate {
  ...
  static Map<String, dynamic>? locateConfiguration(String repoPath) {
    assert(repoPath.isNotEmpty, 'EC-AWCV013A07-001: repo path must not be empty');
  ...
  static Map<String, dynamic> extractParameters(Map<String, dynamic> config) {
    ...
    assert(
      ...
      'EC-AWCV013A07-002: all 5 focus ring fields must be non-null',
    );
  ...
  static FocusRingColorEntry registerRule(FocusRingColorEntry entry) {
    assert(entry.focusRingToken == FocusRingColorEntry.kRequiredToken,
      'EC-AWCV013A07-003: focusRingToken must be md.sys.color.outline');
    assert(entry.contrastRatio >= FocusRingColorEntry.kMinContrastRatio,
      'EC-AWCV013A07-003: contrastRatio < 3.0');
    assert(entry.focusRingWidthPx >= FocusRingColorEntry.kMinWidthPx,
      'EC-AWCV013A07-003: focusRingWidthPx < 2px');
  ...
  static String bindToTarget(String ruleId, String focusRingToken) {
    assert(ruleId.isNotEmpty, 'EC-AWCV013A07-005: FK bind requires valid ruleId');
